...
| [INC-2026-010](./INC-2026-010-pve-dell-outage-single-replica-workbench-outage.md) | 2026-09-12 | SEV-3 | `pve-dell` / `k3s-worker-1` / Longhorn / workbench | A hypervisor outage faulted every single-replica volume on one worker; alerts fired correctly but were discarded, so it ran 47 hours, and Longhorn's manager then crash-looped 41 times on a startup write conflict before recovery could proceed | Resolved; alert delivery (#92) and host root cause open |
