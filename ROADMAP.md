...
- [x] GitOps bootstrap — ArgoCD 10.1.2 (v3.4.4) live, self-managed via app-of-apps (ADR-0042), internal-only at `argocd.in.neovara.uk` (ADR-0043). Traefik migrated as the first GitOps-managed app, proof-of-concept for the rest (ADR-0044) — first sync diff was tracking-annotations only, confirming a clean adoption. Remaining apps (cert-manager, Longhorn, cloudflared, monitoring stack) stay imperative until migrated the same way; tracked as the open remainder of issue #14.
- [ ] Argo Rollouts (progressive delivery) — controller + CRDs staged as a GitOps-native `Application` (`k8s/argocd/apps/argo-rollouts.yaml`, `k8s/argo-rollouts/values.yaml`), no bootstrap paradox and nothing pre-existing to adopt, so it onboards straight into GitOps from the first commit (`CreateNamespace=true`, no manual `kubectl apply` at all) — ADR-0045. Manifests written, commit/push pending. Canary/blue-green usage (a real `Rollout` + `TraefikService`, Traefik's native integration, no plugin needed) deferred until a real app exists to canary — tracked in issue #26.
