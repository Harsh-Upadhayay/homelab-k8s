...
- **HA control plane** (`k3s-server-2`, `k3s-server-3`) and **additional workers beyond the explicitly planned Immich recovery worker** — additive in cluster terms, but each host/VM still belongs in Terraform and Ansible before it joins. See `docs/migrations/immich.md` for that one planned worker.
