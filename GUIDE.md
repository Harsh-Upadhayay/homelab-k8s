...
- **HA control plane** (`k3s-server-2`, `k3s-server-3`) and **more workers** — both are additive. A second/third server joins the existing embedded-etcd cluster for real quorum; each additional worker needs a Terraform resource and an Ansible inventory entry. The current worker pair spans both physical hosts; issue #48 completed worker retirement and storage convergence.
