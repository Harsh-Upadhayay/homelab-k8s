...
- **GitOps (ArgoCD) is live, not deferred anymore.** Bootstrapped via one manual `helm install`, then self-managed via the "app of apps" pattern (ADR-0042), internal-only at `argocd.in.neovara.uk` (ADR-0043). Only Traefik is migrated so far, as a deliberate one-app-at-a-time proof of concept (ADR-0044) — cert-manager, Longhorn, cloudflared, and the monitoring stack are still plain `helm upgrade`/`kubectl apply`, tracked as the open remainder of issue #14. Don't assume "GitOps means everything is in Argo now" — check `kubectl get applications -n argocd` for the current real scope. Argo Rollouts (progressive delivery — canary/blue-green) is also added as a GitOps-native app with no live usage yet (ADR-0045, issue #26).
...
Full backup strategy (off-box etcd shipping, Velero, Proxmox Backup Server), Cilium, HA control plane (`k3s-server-2`/`-3`), and workers beyond the two already provisioned. Each is staged as a clean, additive follow-on — none of them are missing by accident, and none should be silently bootstrapped while working on something else. (Longhorn, monitoring/logging, and GitOps/ArgoCD were all in this list too, until each landed — see the "Decisions already made" section above for their current state.)
